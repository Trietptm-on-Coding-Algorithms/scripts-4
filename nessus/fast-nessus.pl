...
$rate="--max-rate 1000 --min-rate 700 --max-retries 1";
$infile=$ARGV[0];
...
if (! -f "$infile-tcp-nmap.nmap") {
    print "TCP scanning...\n";
    print "nmap -r -n -iL $infile -oA $infile-tcp-nmap $rate -p1-65535\n";
    $nmap=`nmap -r -n -iL $infile -oA $infile-tcp-nmap $rate -p1-65535`;
  
}
else
{
    print "Using cached TCP scan $infile-tcp-nmap.nmap and $infile-udp-nmap.gnmap\n";
}

if (! -f "$infile-udp-nmap.nmap") {
    print "UDP scanning...\n";
    print "nmap -r -n -sU -iL $infile -oA $infile-udp-nmap $rate\n"; 
    $nmap=`nmap -r -n -sU -iL $infile -oA $infile-udp-nmap $rate`;
}
else 
{
